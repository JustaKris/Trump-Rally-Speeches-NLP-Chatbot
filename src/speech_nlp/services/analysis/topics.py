...
from speech_nlp.utils.text import clean_text, tokenize_text
...
Respond with ONLY the label, nothing else."""
    ...
    def _extract_snippet(
        ...
        # Clean up — prefer sentence boundaries, fall back to word boundaries
        if truncated_start:
            # Try to find a sentence boundary (". " or "? " or "! ") in the first half
            sentence_break = re.search(r"[.!?]\s+", snippet[: context_chars // 2])
            if sentence_break:
                snippet = snippet[sentence_break.end() :]
                truncated_start = False  # clean sentence start, no ellipsis needed
            else:
                # Fall back to the first word boundary (space)
                first_space = snippet.find(" ")
                if first_space != -1 and first_space < context_chars // 3:
                    snippet = snippet[first_space + 1 :]

        if truncated_end:
            # Try to find the last sentence boundary in the second half
            half = len(snippet) - context_chars // 2
            matches = list(re.finditer(r"[.!?]\s*", snippet[half:]))
            if matches:
                cut = half + matches[-1].end()
                snippet = snippet[:cut]
                truncated_end = False
            else:
                # Fall back to the last word boundary (space)
                last_space = snippet.rfind(" ")
                if last_space != -1 and last_space > len(snippet) * 2 // 3:
                    snippet = snippet[:last_space]

        # Add ellipsis for mid-text truncations
        if truncated_start:
            snippet = "..." + snippet
        if truncated_end:
            snippet = snippet + "..."
