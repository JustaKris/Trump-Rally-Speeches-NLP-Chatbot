...
1. **Check Startup Logs**
   ...
   Look for the critical error block in your AWS logs:
   ```
...
Before deploying to AWS, test the exact production configuration locally:

```powershell
# Use production environment
$env:ENVIRONMENT="production"

# Test with Docker (same as AWS runs)
...
If it works locally with `ENVIRONMENT=production`, but fails in AWS, the issue is AWS-specific (likely environment variables or resource constraints).
