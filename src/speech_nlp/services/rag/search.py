...
class SearchEngine:
    ...
    def search(self, query: str, top_k: int = 5) -> List[SearchResult]:
        ...
        if self.use_hybrid_search and self.bm25 is not None:
            logger.debug("Hybrid search for query: %r (top_k=%d)", query, top_k)
            return self._hybrid_search(query, top_k)
        else:
            logger.debug("Semantic search for query: %r (top_k=%d)", query, top_k)
    ...
    def _hybrid_search(self, query: str, top_k: int) -> List[SearchResult]:
        ...
        logger.debug(
            "Hybrid search merged %d semantic + %d BM25 → %d unique results",
